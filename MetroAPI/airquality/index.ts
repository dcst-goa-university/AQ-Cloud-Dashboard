...
export async function airquality(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  ...
  if (!lat || !lon) {
    return { status: 400, jsonBody: { error: "lat and lon required" } };
  }
  ...
  try {
    ...
    context.log(`Error fetching air quality: ${err.message}`);
    return { status: 500, jsonBody: { error: "Failed to fetch data" } };
  }
}
